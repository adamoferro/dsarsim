...
def main(argv=None):
    ...
    parser.add_argument('-pa','--azimuthPixelSpacing',type=float,default='0',help="spacing between azimuth pixels in meters, or 0 if automatically set as the same spacing of the input DTM/DSM. Default is 0")
    parser.add_argument('-pr','--slantRangePixelSpacing',type=float,default='0',help="spacing between slant range pixels in meters, or 0 if automatically calculated in order to obtain a flat-terrain-'ground range' simulation. Default is 0")
    parser.add_argument('-d','--direction',default='w',choices=['w','e'],help="direction of view of the sensor. 'w' = West to East, 'e' = East to West (assuming the sensor going from South to North with aspect angle = 0). Default is 'w'")
    ...
    parser.add_argument('-r','--rotateBack',default=False,action='store_true',help='if aspect angle != 0, rotate back output image by -aspect angle. Use only if azimuthPixelSpacing and slantRangePixelSpacing are not set by user in order to get simulations directly comparable to input DTM/DSM. Default is not set')
